...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-2xl border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-primary-foreground shadow-sm shadow-primary/20 hover:bg-primary/85",
        outline:
        ...
        gradient:
          "bg-gradient-to-r from-violet-600 via-fuchsia-600 to-purple-600 text-white font-semibold shadow-lg shadow-violet-500/25 hover:from-violet-500 hover:via-fuchsia-500 hover:to-purple-500 active:scale-[0.98] disabled:scale-100 overflow-hidden relative",
      },
      size: {
        default:
...
function Button({
  ...
  return (
    ...
    >
      {variant === "gradient" && (
        <div className="absolute inset-0 bg-[linear-gradient(60deg,transparent,rgba(255,255,255,0.12),transparent)] translate-x-[-100%] group-hover/button:translate-x-[100%] transition-transform duration-700" />
      )}
      {children}
    </Comp>
  )
